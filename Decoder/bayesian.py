...
class HybridDecoderBO:
    ...
    def evaluate(self, bg_weights):
        ...
        V_OL_TARGET = 0.5  # Target for Logic 0 (Maximum)
        V_OH_CAP = 3.4
        ...
        for ops in self.test_ops: 
            ...
            try:
                ...
                max_high = np.max(high_volts)
                # --- TARGET-BASED NORMALIZATION --

                # 1. Extinction/Gap (Targeting separation)
                actual_gap = min_high - max_low
                er_score = np.tanh(actual_gap / GAP_TARGET)

                # 2. Strength (High Voltage level)
                high_cap_penalty = np.exp(-max(0, max_high - V_OH_CAP) / 0.3)
                str_score = np.tanh(min_high / V_OH_TARGET) * high_cap_penalty
                #str_score = np.clip(min_high / V_OH_TARGET, 0, 1)

                # 3. Dedicated Consistency & Low Penalty
                std_high = np.std(high_volts)
                std_low = np.std(low_volts)

                # Specifically penalize the spread of high logic
                # A spread of > 0.5V should start hurting the score significantly
                high_uniformity = np.exp(-std_high / 0.06) 
                low_cleanliness = np.exp(-max(0, max_low - V_OL_TARGET) / 0.5)

                # Combine for a more granular Consistency Score
                # You can adjust the 0.7/0.3 balance to care more about High vs Low
                cons_score = (0.7 * high_uniformity + 0.3 * np.exp(-std_low / 0.2)) * low_cleanliness
    ...
    def optimize(self, total_cycles=10):
        ...
        for cycle in range(total_cycles):
            ...
            n_cands = 500 
            cands_rand = np.random.uniform(0.1, 4.9, (int(n_cands*0.8), USE))
            ...
            beta = 3.0 if self.best_score < 0.3 else 1.5
            ...
            if (cycle + 1) % 2 == 0:
                ...
                self.explore_around_best(n_samples=8, radius=0.5)
