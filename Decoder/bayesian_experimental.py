...
MODIFIABLE_HEATERS = [h for h in range(14) if h not in INPUT_HEATERS] # just 2 layers
...
# ============================================================
def get_test_suite():    
    # Only include the instructions defined in your logic
    #   inputs
    return [
        (1, 0, 1, 1, 1), # Instruction 1: Out IM
        (1, 1, 1, 1, 1)  # Instruction 2: JMP
    ]
...
class HybridDecoderBO:
    ...
    def evaluate(self, bg_weights):
        ...
        time.sleep(0.12) # Slightly increased sleep for thermal stabilization
        
        # --- OPTIMIZATION TARGETS ---
        V_TARGET_HIGH = 3.3  # The ideal "Logic 1" level
        V_TARGET_LOW = 0.3   # The ideal "Logic 0" level
        GAP_TARGET = V_TARGET_HIGH - V_TARGET_LOW     # Desired separation (5.0 - 1.8)
        V_LIMIT = 5  # The absolute maximum safe voltage for your hardware

        # Component Weights
        W_ER = 0.4    # Extinction Ratio / Gap
        W_STR = 0.3   # Voltage Strength (Prioritized)
        W_CONS = 0.3  # Consistency & Low-level cleanliness
        ...
        for ops in self.test_ops: 
            ...
            try:
                ...
                high_volts = vals[targets == 1]
                low_volts = vals[targets == 0]
                ...
                std_high = np.std(high_volts)

                # --- REVISED STRENGTH SCORE (The 3.3V Cap) ---
                
                if min_high < V_TARGET_HIGH:
                    # Standard push toward 3.3V
                    str_score = (min_high / V_TARGET_HIGH) ** 4 
                else:
                    # We are above 3.3V. We want to stay near 3.3V, not go to 6V!
                    # This uses a Gaussian-style penalty: it peaks at 3.3V and drops if it goes higher.
                    over_voltage_penalty = np.exp(-((min_high - V_TARGET_HIGH) ** 2) / 0.5)
                    str_score = 1.0 * over_voltage_penalty

                # --- HARD CAP PENALTY ---
                # If any single channel exceeds our safe hardware limit, crash the FOM
                if np.max(vals) > V_LIMIT:
                    str_score -= 2.0  # Force the BO to move away from this configuration

                # --- 2. EXTINCTION / GAP SCORE ---
                actual_gap = min_high - max_low
                er_score = np.tanh(actual_gap / GAP_TARGET)

                # --- 3. CONSISTENCY & CLEANLINESS ---
                # Penalize high logic spread (std_high) and high logic 0 levels (max_low)
                high_uniformity = np.exp(-std_high / 0.05) 
                low_cleanliness = np.exp(-max(0, max_low - V_TARGET_LOW) / 0.4)
                cons_score = high_uniformity * low_cleanliness

                # Final Figure of Merit for this instruction
        ...
        # Optimization returns the worst-performing instruction to ensure all pass
        return float(np.min(instruction_scores))

    def fit_gp(self):
        if len(self.X) < 10: return None
        n_dims = len(MODIFIABLE_HEATERS)
        kernel = (ConstantKernel(1.0) * Matern(length_scale=[1]*n_dims, nu=2.5) + 
    ...
    def explore_around_best(self, n_samples=15, radius=0.3):
        ...
        for i in range(n_samples):
            # Tweak x% of heaters randomly
            indices = np.random.choice(len(MODIFIABLE_HEATERS), int(USE * 1.0), replace=False)
            new_x = base_x.copy()
    ...
    def optimize(self, total_cycles=50):
        # 1. Initial Sampling
        print("Starting Initial Latin Hypercube Sampling...")
        sampler = qmc.LatinHypercube(d=len(MODIFIABLE_HEATERS))
        for x in (sampler.random(75) * 4.5 + 0.1):
        ...
        for cycle in range(total_cycles):
            ...
            n_cands = 1500 
            candidates = np.random.uniform(0.1, 4.9, (int(n_cands), USE))
            ...
            beta = 3.0 if self.best_score < 0.3 else 1.0
            mu, sigma = gp.predict(candidates, return_std=True)
            ucb_values = mu + beta * sigma
            
            # Get the top 3 unique suggestions from the BO
            best_indices = np.argsort(ucb_values)[-4:][::-1] 
            ...
            if (cycle + 1) % 1 == 0:
                ...
                self.explore_around_best(n_samples=6, radius=0.5)
